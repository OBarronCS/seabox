...
PARAM_MOUNT_DIRECTORY="#INSERT_MOUNT_DIRECTORY#"
...
then
    if [ -z "$PARAM_MOUNT_DIRECTORY" ] || ! su --pty --help >/dev/null 2>&1;
    then
        exec su -s "$SHELL" - "$USERNAME"
    else
        exec su --pty -s "$SHELL" - "$USERNAME" -c "cd $PARAM_MOUNT_DIRECTORY && exec $SHELL"
    fi
